...
public class TesterScreenProvider extends AbstractProvider {
                ...
                (Table source, Object itemId, Object columnId) -> {
                    ...
                    && itemId instanceof String) {
                        ...
                        if (locked) {
                            ...
                            label2.setDescription(getUI().translate("message.locked"));
                        }
                        if (!summary.isEmpty()) {
                            ...
                            label.setCaption(getUI().translate(message));
                            label.setDescription(getUI().translate(message));
                            ...
                            tp.getTestCaseList().forEach(testCase -> {
                                ...
                                testCase.getStepList().forEach(s -> {
                                    s.getExecutionStepList().forEach(es -> {
                                        ...
                                        if (es.getAssignee().getId().equals(getUI().getUser().getId())) {
    ...
    private void showExecutionScreen(List<TestCaseExecutionServer> executions,
        ...
        if (executionWindow == null) {
            executionWindow = new ExecutionWindow(getUI(), executions, tcID);
        ...
        if (!getUI().getWindows().contains(executionWindow)) {
            getUI().addWindow(executionWindow);
        }
    }
}
