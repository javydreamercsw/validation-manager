...
public class CauseServer extends Cause implements EntityServer{
    ...
    public int write2DB() throws NonexistentEntityException, Exception {
        if (getId() > 0) {
            Cause c = new CauseJpaController( DataBaseManager.getEntityManagerFactory()).findCause(getId());
            c.setDescription(getDescription());
            c.setName(getName());
            if (getRiskItemList() != null) {
                c.setRiskItemList(getRiskItemList());
            }
            new CauseJpaController( DataBaseManager.getEntityManagerFactory()).edit(c);
        } else {
            Cause c = new Cause();
            c.setDescription(getDescription());
            c.setName(getName());
            c.setRiskItemList(getRiskItemList());
            new CauseJpaController( DataBaseManager.getEntityManagerFactory()).create(c);
            setId(c.getId());
        }
        return getId();
    }
    ...
    public static boolean deleteCause(Cause c) throws NonexistentEntityException {
        new CauseJpaController( DataBaseManager.getEntityManagerFactory()).destroy(c.getId());
        return true;
    }
}
